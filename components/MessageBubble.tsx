...
const MessageBubble: React.FC<MessageBubbleProps> = ({ message }) => {
  ...
  return (
    <div className={`${containerClasses} mb-2`}>
      <div className={`p-3 max-w-xl shadow ${bubbleClasses}`}>
        {message.isLoading && message.sender === 'ai' && !message.text ? (
          <div className="flex items-center">
            <LoadingSpinner size="sm" /> <span className="ml-2 text-sm">Thinking...</span>
          </div>
        ) : (
          <p className="whitespace-pre-wrap">{message.text || (message.isLoading ? "..." : "")}</p>
        )}
